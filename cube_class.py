...
class Cube:
    ...
    def img_process_canny2max_contour(self, canny_image):
        ...
        max_cont = max([len(c) for c in contours]) # contour with most points
        contours_corners = [c for c in contours if len(c)==max_cont][0] # most point = has to be a corner
...
def main():    
    img = cv2.imread("data/blue_3.jpg")
